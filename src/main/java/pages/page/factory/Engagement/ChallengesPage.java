package pages.page.factory.Engagement;
...
public class ChallengesPage extends GenericPage {
    ...
    @FindBy(xpath = "//*[@class='v-form']//following::*[@class='flex drawerFooter border-box-sizing flex-grow-0 px-4 py-3']//button[2]")
    ...
    private BaseElementFacade getSelectSpaceInDropDown(String spaceName) {
        return findByXpath(String.format(
                "//div[contains(@class,'identitySuggestionMenuItemText') and contains(text(),'%s')]", spaceName));
    }


    private BaseElementFacade getSelectDomainInDropDown(String programName) {
        return findByXpath(String.format(
                "//div[contains(@class,'identitySuggestionMenuItemText') and contains(text(),'%s')]", programName));
    }
    ...
    private BaseElementFacade dateFiled;

    public void checkDateField(String title) {
        getChallengeCardTitle(title).isVisibleAfterWaiting();
        dateFiled.shouldBeVisible();
    }
    ...
    @FindBy(xpath = "//*[@class='flex drawerContent flex-grow-1 overflow-auto border-box-sizing']//following::*[@class='flex drawerFooter border-box-sizing flex-grow-0 px-4 py-3']//button[2]")
    ...
    private BaseElementFacade getSelectWinnerInDropDown(String secondUserName) {
        return findByXpath(String.format("//div[contains(@class,'identitySuggestionMenuItemText') and contains(text(),'%s')]", secondUserName));
    }
    ...
    private BaseElementFacade getAnnouncementActivityTopBar(String user, String space) {
        return findByXpath(String.format("(//*[contains(@class,'accountTitleLabel')][1]//*[contains(@id,'userAvatar')and contains(@href,'%s')]/following::*[contains(@id,'spaceAvatar') and contains(text(),'%s')])[1]", user, space));
    }
    ...
    private BaseElementFacade getWinnerNameOnAnnouncement(String user) {
        return findByXpath(String.format("//*[@class='font-weight-bold text-color mx-0 mt-0 mb-2 text-wrap text-break text-truncate-2']/a[contains(@href,'%s')]", user));
    }
    ...
    private BaseElementFacade getChallengeTitleOnAnnouncement(String name) {
        return findByXpath(String.format("//*[@class='caption text-wrap text-break rich-editor-content reset-style-box text-light-color text-truncate-3']/a[contains(text(),'%s')]", name));
    }
